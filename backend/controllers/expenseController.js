...
exports.processSMS = async (req, res) => {
  try {
    ...
    // Check if SMS is likely a transaction using simple keywords
    if (SMSParser.isTransactionSMS(smsBody)) {
      // Parse SMS with Regex first
      parsedData = SMSParser.parseTransaction(smsBody);
    } else {
      console.log('Not a recognized transaction SMS format by regex keywords. Trying Gemini fallback...');
    }

    // If regex parser failed or gave no valid data, try Gemini
    if (!parsedData || !parsedData.isValid) {
      console.log('Trying Gemini fallback parser...');
      parsedData = await parseSMSWithGemini(smsBody);
    }

    if (!parsedData || !parsedData.isValid) {
      return res.status(200).json({
        success: true,
        message: 'Could not parse transaction from SMS even with AI fallback',
        isTransaction: false
      });
    }

    // Check for duplicate transaction (within 5 minutes)
    const transactionDate = parsedData.transactionDate ? new Date(parsedData.transactionDate) : new Date(timestamp || Date.now());
    ...
    const expenseData = {
      ...
      category: autoCategorize ?
        (parsedData.category || SMSParser.categorizeExpense(parsedData.merchant)) : 'Other',
